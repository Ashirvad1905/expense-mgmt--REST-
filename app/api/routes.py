...
from app.crud import users, categories, expenses, budgets
...
def create_expense(data: ExpenseIn, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
...
def update_expense(expense_id: int, data: ExpenseIn, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
...
def create_budget(data: BudgetIn, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
...
def update_budget(budget_id: int, data: BudgetIn, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
